    ...
    def wrapper(func):
        ...
        def decorator(*args, **kwargs):
            ...
            if not token or not app:
                app_permissions = encode_numbers([APIPermissions.get_user])
                allowed_user_fields = encode_numbers([1])
                app_id = "unknown"
                app_name = "unknown"
            else:
                app_permissions = app.api_permissions
                allowed_user_fields = app.allowed_user_fields
                app_id = app.id
                app_name = app.name

            metric_labels = [app_id, app_name, request.path]

            if required_permission not in decode_number(app_permissions):
                apps_requests.labels(*metric_labels, "Access denied").inc()
                raise errors.AccessDeniedError

            apps_requests.labels(*metric_labels, "Success").inc()
            return func(*args, **kwargs, user_fields=decode_number(allowed_user_fields))
